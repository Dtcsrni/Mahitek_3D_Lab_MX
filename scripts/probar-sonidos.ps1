...
# Reproduce todos los sonidos disponibles para verificar configuracion

# Cargar libreria de sonidos
$sonidosPath = Join-Path $PSScriptRoot "lib\sonidos.ps1"

if (-not (Test-Path $sonidosPath)) {
    Write-Host "[ERROR] No se encontro la libreria de sonidos en: $sonidosPath" -ForegroundColor Red
    exit 1
}
...
Write-Host "Esta prueba reproducira todos los sonidos disponibles." -ForegroundColor White
...
Write-Host "Si no escuchaste ningun sonido:" -ForegroundColor Yellow
Write-Host "  1. Verifica que tu PC tenga altavoces/audifonos conectados" -ForegroundColor White
