/**
 * Renderizado de enlaces sociales.
 */
...
import { validateBrandData, validateSocialData } from './validation.js';
...
export async function initSocialLinks() {
  ...
  if (brand) {
    addHealthReport('brand', validateBrandData(brand));
  }
  if (!social) {
    social = await loadJSON(CONFIG.DATA_PATHS.social);
  }
  const report = validateSocialData(social);
  if (!social) {
    addHealthReport('social', report);
    return;
  }
  ...
  const buildLinks = socialData => {
    const links = [];
    pushIfValid(links, { key: 'instagram', label: 'Instagram', url: socialData.instagram });
    pushIfValid(links, { key: 'facebook', label: 'Facebook', url: socialData.facebook });
    pushIfValid(links, { key: 'tiktok', label: 'TikTok', url: socialData.tiktok });
    pushIfValid(links, { key: 'youtube', label: 'YouTube', url: socialData.youtube });
    pushIfValid(links, { key: 'twitter', label: 'X', url: socialData.twitter });
    return links;
  };
  ...
  if (container) {
    container.innerHTML = links
      .map(
        link => `
      <a class="social-icon social-icon--${link.key}" href="${sanitizeURL(
        link.url
  ...
  if (heroContainer) {
    heroContainer.innerHTML = links
      .map(
        link => `
      <a class="community-link" href="${sanitizeURL(link.url)}" target="_blank" rel="noopener noreferrer" aria-label="${escapeHTML(
        link.label
      )}">
        ${getSocialIcon(link.key)}
  ...
  report.summary.renderizadas = links.length;
  report.summary.footer = container ? links.length : 0;
  report.summary.hero = heroContainer ? links.length : 0;
  addHealthReport('social', report);
}
